...
static void			delete_edge_from_item_dfs_sink(t_hash_item *temp_item, char *btl_room)
{
...
void				ft_cut_edge_ds(t_ant_farm *ant_farm, \
								t_paths *tmp_path, \
								t_room *bottleneck_room)
{
	...
	while (tmp_pathlst != NULL)
	{
		if (tmp_pathlst->room->name == bottleneck_room->name)
		{
			...
			delete_edge_from_item_dfs_sink(room_item, bottleneck_room->name);
		}
		tmp_pathlst = tmp_pathlst->next;
	}
}

void				ft_set_score_pathlst(t_path_list *path_lst)
{
...
void				ft_set_score_zero(t_ant_farm *ant_farm, int path_id)
{
	t_paths			*curr_node;
	// t_paths			*tmp_next;
...
int					find_path_cut_edge_ds(t_ant_farm *ant_farm, \
							t_room *bottleneck_room)
{
	t_paths			*tmp_path;
									...
									t_neighbor *neighbors, t_room *temp)
{
	...
	while (neighb_tmp != NULL)
	{
		...
		// temp->position != START)
		{
			...
			path_id = find_path_cut_edge_ds(ant_farm, neighb_tmp->hash_item->room);
